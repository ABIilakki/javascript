...
for(let i=0;i<electronicItems.length;i++){
    let found=false
    for(let j=0;j<originalslength;j++){
            if(originals[j]===electronicItems[i].category){
              found=true
              break
            }
        }
        if(!found){
            originals[originalslength] = electronicItems[i].category;
            originalslength = originalslength + 1;
        }
    }
 ...
 for(let k=0;k<originals.length;k++){
     item[originals[k]]=[];
     for(let l=0;l<electronicItems.length;l++){
     if(electronicItems[l].category===originals[k]){
         item[originals[k]].push(electronicItems[l])
    }
}

for(let m=0;m<electronicItems.length;m++){
    let items= electronicItems[m];
    let price=electronicItems[m].price;
    let discount=0;
    let Gst=0;
    let totalpaid=0;
