...
//     }
// })
//     details.then((name)=>{console.log(name)})
//     details.catch((error)=>{console.log(error)})
//     details.finally(()=>{console.log('done')})
let email='ssbinddd@gmail.com.in';
...
Promise.all([
  checkCapitalLetter(email),
  checkSpecialChars(userName),
  checkLength(userName),
  checkIndexs(userName),
  checkContinousDot(userName),
  checkContinousUnderscore(userName),
  checkNumbers(domainName),
  checkIndex(domainName),
  checkContinousDots(domainName),
  checkDotLimit(domainName),
  checkAtLimit(email)
])
.then(messgae => {
    console.log("valid");
